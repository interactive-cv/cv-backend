...
async def edit_chat(
    ...
    if body.mode == "chat":
        prompt += (
            ...
            "Если пользователь просит изменить тексты — предложи конкретную "
            "правку словами (что именно изменишь) и скажи, что применишь её "
            "по подтверждению. В этом ответе правку не выполняй."
        )
    else:
        prompt += (
