...
async def test_refuses_fake_employer():
    ...
    a = answer.lower()
    # Не должен утвердительно упомянуть Google как реальный опыт работы.
    ...
    # Должен отказаться / признать отсутствие данных (любой формулировкой).
    # Маркеры отказа: упоминание CV + отсутствие, либо прямое «не работал».
    refusal_markers = ["нет в cv", "в cv нет", "cv этой информации нет", "в cv не",
                       "не работал", "не подтверждаю", "такой информации нет", "не указано",
                       "информации нет", "отсутствует", "не могу подтвердить"]
    assert any(s in a for s in refusal_markers), f"AI не отказался от выдумки чётко: {answer!r}"
